...
class my_evaluation:
    ...
    def confusion(self):
        ...
        for label in self.classes_:
            ...
            for i in range(len(self.actuals)):
                compareList.append([self.predictions[i],self.actuals[i]])
            compareList = np.array(compareList)
            guessedLabel = []
            guessedWrong = []

            for i in range(len(compareList)):
                if compareList[i][0] == label:
                    guessedLabel.append(compareList[i])
                else:
                    guessedWrong.append(compareList[i])
            guessedLabel = np.array(guessedLabel)
            guessedWrong = np.array(guessedWrong)

            numLabelGuessed = len(guessedLabel)
            numNonlabelGuessed = len(guessedWrong)
            numPosCorrect = 0
            numNegCorrect = 0
            for i in guessedLabel:
                if i[0] == i[1]:
                    numPosCorrect += 1
            for i in guessedWrong:
                if i[0] == i[1]:
                    numNegCorrect += 1

            tp =  numPosCorrect
            fp =  numLabelGuessed - tp
            tn =  numNegCorrect
            fn =  numNonlabelGuessed - tn
    ...
    def recall(self, target=None, average = "macro"):
        ...
        if average == "micro":
            ...
            for i in self.classes_:
                count = 0
                for j in self.actuals:
                    if j == i:
                        count += 1
                f1_score = self.f1(target=i)
                f1_score *= count
                total += f1_score
            total /= len(self.classes_)
            return total
        elif target == None:
            #do total recall
            allTP = 0
            allFP = 0
            for i in self.classes_:
                allTP += self.confusion_matrix[i]["TP"]
                allFP += self.confusion_matrix[i]["FP"]
            rec = allTP / (allTP + allFP)

        else:
            tpLoc = self.confusion_matrix[target]["TP"]
            rec = tpLoc / (tpLoc + self.confusion_matrix[target]["FP"])
    ...
    def f1(self, target=None, average = "macro"):
        ...
        #NEED TO ADD MACRO,MICRO,WEIGHTED
        if average == "micro":
